...
class Engine {
  constructor(params) {
    ...
    this.errorHandlerInit = this.errorHandlerInit.bind(this);
  ...
  errorHandlerInit() {
    ...
    const safelyBuildEvent = (error) => {
      let event = null;
      try {
        event = this.buildEvent(`puck ${error}`);
      } catch(error) {
        event = error;
      }
    ...
    this.socket.on('error', (error) => {
      this.onError('error', error, safelyBuildEvent());
    });

    this.socket.on('connect_error', (error) => {
      this.onError('connect_error', error, safelyBuildEvent());
    });
  ...
  /**
   * Construct a fully formed event object.
   */
  buildEvent(name, data = null) {
    const {
