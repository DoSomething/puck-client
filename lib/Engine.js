...
import {
  ...
  getUrlData, getBrowserSize, getPuckCookie,
} from './helpers';
...
class Engine {
  constructor(params) {
    ...
    this.updateCookieId = this.updateCookieId.bind(this);
    this.updateCookieId();
  ...
  /**
   * Update the cookie id.
   */
  updateCookieId() {
    this.cookieId = getPuckCookie();

    if (! this.cookieId) {
      this.fetch(`${this.puckUrl}/id`).then(this.updateCookieId);
    }
  }
  ...
  dispatchEvent(name, data = null) {
    const {
      source, getUser, deviceId, cookieId,
